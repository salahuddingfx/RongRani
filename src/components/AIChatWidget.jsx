...
const AIChatWidget = () => {
  ...
  const getBotResponse = (userMessage) => {
    ...
    if (msg.includes('love') || msg.includes('romantic')) {
      ...
      return "Call us: +8801851075537 or email: salauddinkaderappy@gmail.com. We're here to help! 📞";
    ...
    <>
      ...
      {!isOpen && (
        <button
          ...
          className="fixed bottom-6 right-6 z-50 bg-maroon text-white p-4 rounded-full shadow-2xl hover:bg-maroon-light transition-all duration-300 hover:scale-110"
        >
      ...
      {/* Chat Widget */}
      {isOpen && (
        <div className="fixed bottom-6 right-6 z-50 w-96 max-w-[calc(100vw-3rem)] h-[600px] max-h-[calc(100vh-3rem)] bg-white rounded-2xl shadow-2xl flex flex-col overflow-hidden border-2 border-maroon/20">
          {/* Header */}
          <div className="bg-maroon text-white p-4 flex items-center justify-between">
            <div className="flex items-center space-x-3">
              <div className="w-10 h-10 bg-white rounded-full flex items-center justify-center shadow-md">
                <Bot className="h-6 w-6 text-maroon" />
              </div>
              <div>
                <h3 className="font-bold">Gift Assistant</h3>
                <p className="text-xs text-white/80">Always here to help ❤️</p>
              </div>
            </div>
            <button
          ...
          {/* Quick Replies */}
          {messages.length <= 2 && (
            <div className="px-4 py-2 border-t border-slate/10 bg-white">
          ...
          <div className="p-4 border-t border-slate/10 bg-white">
            <div className="flex items-center space-x-2">
              <input
                ...
                className="flex-1 bg-pink-50/50 rounded-full px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-maroon/30"
              />
              <button
                onClick={handleSendMessage}
                disabled={!inputMessage.trim()}
                className="bg-maroon text-white p-3 rounded-full hover:bg-maroon-light disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
              >
